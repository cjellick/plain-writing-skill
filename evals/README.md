...
<tr>
<td valign="top">Done. `assignment_updates.py` is the only new file, `python3 test_assignment_update.py` ends with `OK — all assignment-update checks passed` (exit 0), and `hcm_transport.py`, `assignment_reader.py`, `test_assignment_update.py`, and everything under `docs/` are untouched.<br><br>## Documentation conflict — called out, protected files left alone<br><br>There is a material discrepancy between the live assignment PATCH endpoint page and the pinned contract/general date-effective topics on range-date requiredness. `docs/contract.json:19-20` (and the general Date-Effective Correction/Update topics) treat RangeStartDate/RangeEndDate as optional for CORRECTION (defaulting to the row's own dates), with only this toolkit's UPDATE requiring an explicit start. The [endpoint page](https://docs.oracle.com/en/cloud/saas/human-resources/farws/op-workers-workersuniqid-child-workrelationships-periodofserviceid-child-<br><br>[...]</td>
<td valign="top">Implemented `assignment_updates.py`. It is the only new file. I did not modify `hcm_transport.py`, `assignment_reader.py`, `test_assignment_update.py`, or anything under `docs/`.<br><br>Verification passed:<br><br>```bash<br>python3 test_assignment_update.py<br># OK — all assignment-update checks passed<br>```<br><br>The command exited with status 0.<br><br>## Documentation reviewed<br><br>I used only first-party Oracle documentation on `docs.oracle.com`.<br><br>- Assignment PATCH endpoint: `PATCH /workers/{workersUniqID}/child/workRelationships/{PeriodOfServiceId}/child/assignments/{assignmentsUniqID}`. The date-effective header is `Effective-Of`, with semicolon-separated parameters such as `RangeMode`, `RangeStartDate`, and `RangeEndDate`. Range dates use `yyyy-MM-dd`.<br>  - https://docs.oracle.com/en/cloud/saas/human-resources/farws/op-workers-workersuniqid-child-workrelationships-periodofserviceid-child-assignments-assignmentsuni<br><br>[...]</td>
<td valign="top">`assignment_updates.py` is complete and is the only new file. I did not change `hcm_transport.py`, `assignment_reader.py`, `test_assignment_update.py`, or any file under `docs/`.<br><br>`python3 test_assignment_update.py` passes and ends with:<br><br>```text<br>OK — all assignment-update checks passed<br>```<br><br>Exit code: 0.<br><br>## Documentation conflict<br><br>The live Oracle assignment PATCH page does not fully match the pinned contract in `docs/contract.json`.<br><br>The pinned contract and the general Oracle date-effective topics say that `RangeStartDate` and `RangeEndDate` are optional for `CORRECTION`. If the dates are omitted, Oracle uses the row's own effective dates. The toolkit also requires an explicit `range_start` for `UPDATE`, so the caller must choose the start date for the new row.<br><br>The assignment PATCH endpoint page I fetched only states that `RangeStartDate` and `RangeEndDate` must use `yyyy-MM-dd` forma<br><br>[...]</td>
</tr>
</tbody>
</table>
